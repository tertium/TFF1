...
use a special binary predicate to encode type information. To avoid mixing
types and terms in the encoded problem, we introduce two sorts, $\typ$ and
$\und$, giving the symbols we generate for type arguments the type $\typ$ while
...
\hat{v}_1,\dots,\hat{v}_m), \MON(\tau)).
$$
We denote with $\AxD$ the set of typing axioms
for all function symbols occurring in $\DEL$.
...
Then we define
$$
\MON(\DEL) \eqdef \{ \MON(\varphi) \vb \varphi \in \DEL \} \cup
\AxD \cup \{ \Inh \}.
$$
...
\begin{theorem}[Soundness of $\BOLDMON$] \label{thm:mon_sound}
...
Every function symbol $\hat{\kappa}$ (function symbol $\hat{f}$,
...
We are going to show that for every type $\sigma$, term $t$,
...
We have three non-trivial cases: equality, quantification over a term
