...
A function symbol with the type signature
\begin{center}
...
\paragraph{Type checking.} In a closed TFF formula, every
term has a unique first-order type. The type of a variable is
determined at the moment of binding. The type of a function
application is determined according to the type signature
of the function symbol and the type arguments of the application.
Assuming a function symbol $f$ has the type signature
$\forall \alpha_1\dots\alpha_n (T_1 \times \dots \times T_m \to T)$,
the type of a term
$f(S_1,\dots,S_n,t_1,\dots,t_m)$ is
$T\pi$, where $\pi$ is the type substitution
$[S_1/\alpha_1,\dots,S_n/\alpha_n]$.
This function application is {\em well-typed\/}
whenever every term argument $t_i$ has the type $T_i\pi$.

Likewise, if a predicate symbol $p$ has the signature
$\forall \alpha_1\dots\alpha_n (T_1 \times \dots \times T_m
\to \text{\tt \$o})$, then a predicate application
$p(S_1,\dots,S_n,t_1,\dots,t_m)$ is {\em well-typed\/}
whenever every term argument $t_i$ has the type
$T_i[S_1/\alpha_1,\dots,S_n/\alpha_n]$.
%
An equality application $t_1 \approx t_2$ is {\em well-typed\/}
whenever the terms $t_1$ and $t_2$ have the same type.

A closed TFF formula is {\em well-typed\/} if and only if
every function, predicate, and equality application in it
is well-typed. We restrict this definition to closed formulas
because the type of a free variable is unspecified.
In what follows, we suppose to deal only with well-typed formulas.
