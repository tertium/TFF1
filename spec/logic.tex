...
First of all, we choose a non-empty set $\univ$ containing
individual values and a non-empty set $\sorts$, whose elements
are called {\em sorts} and represent monomorphic first-order types.
To every sort $X \in \sorts$ we assign a non-empty {\em domain}
$\dom_{\cI} X \subseteq \univ$.
...
a sort. We evaluate first-order types according to the
following equations:
\begin{align*}
...
to denote the evaluation of a monomorphic type $S$. We also write
$\evalG{T}_{[\alpha_1 \mapsto X_1,\dots,\alpha_n \mapsto X_n]}$
to denote the evaluation of a type $T$ such that every type
variable in $T$ belongs to $\{ \alpha_1,\dots,\alpha_n \}$.
...
maps any $n$ sorts $X_1,\dots,X_n$ and
any $m$ elements of $\univ$
to an element of
$\dom_{\cI} \evalG{T}_{[\alpha_1 \mapsto X_1,\dots,\alpha_n \mapsto X_n]}$.
...
\bigwedge_{a \in \dom_{\cI} \evalT{T}} \!\!\!\!
\llbracket F \rrbracket^\cI_{\pi,\xi[u\,\mapsto\,a]} &
\eval{\forall \alpha \,.\, F} &\eqdef
\bigwedge_{X \in \sorts}\;
\llbracket F \rrbracket^\cI_{\theta[\alpha\,\mapsto\,X],\xi}
\end{align*}
...
Likewise, $\theta[\alpha \mapsto X]$ is the function that maps
type variable $\alpha$ to the sort $X$ and every other type
