...
When applied to terms and formulas, type substitutions instantiate
types in variable bindings and in type arguments of function and
predicate symbols. Bound type variables are not instantiated.
...
Let $\gamma$ be a {\em type context}, a function that assigns
to every variable symbol a first-order type.
We write $\gamma[v \mapsto S]$ to denote a function that maps the
variable $v$ to type $S$ and every other variable $u$ to $\gamma(u)$.
Given a type substitution $\tau$, we write $\gamma\tau$ to denote
a function that maps every variable $u$ to $\gamma(u)\tau$.

A {\em type judgement\/} $\gamma \vdash t \ty T$ means that term $t$
is {\em well-typed\/} and has type $T$ in the type context $\gamma$.
A judgement $\gamma \vdash F \ty \omicron$ means that formula $F$ is
{\em well-typed\/} in context $\gamma$.
...
occurs neither in $F$ nor among the values of $\gamma$.
...
\frac{\gamma[\alpha'/\alpha] \,\vdash\, F \ty \omicron}{
...
well-typed in one typing context, it is well-typed in any other.
Thus, in what follows, we will omit $\gamma$ and simply write
${} \vdash F \ty \omicron$.
A closed well-typed formula is called {\em sentence}.
...
\llb F[\alpha'/\alpha] \rrb^\cI_{\gamma,\theta[\alpha'\,\mapsto\,D],\xi}
\end{align*}
