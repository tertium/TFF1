...
$\mathbb{Q}$, and $\mathbb{R}$, respectively.
...
implementation to restrict itself to ``pure TFF\kern.04ex$k$''---i.e., TFF\kern.04ex$k$
without arithmetic. For clarity, TFF\kern.04ex$k$ with arithmetic is sometimes
labeled ``TFA\kern.04ex$k$.''
...
every type variable occurring in the body $\varsigma$
