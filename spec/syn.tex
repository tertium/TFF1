...
\ourparagraph{Example.} The following TFF1 problem axiomatizes {\tt lookup} and
{\tt update} operations on maps and conjectures that {\tt update} is idempotent
...
tff(lookup_update_same, axiom,
    (![M : map(B, A), K : B, V : A]:
         lookup(A, B, update(A, B, M, K, V), K) = V)).
tff(lookup_update_other, axiom,
    (![M : map(A, B), V : B, K1 : A, K2 : A]:
         (K1 != K2 => lookup(A, B, update(A, B, M, K1, V), K2) =
                      lookup(A, B, M, K2)))).
tff(map_ext, axiom,
    (![M1 : map(A, B), M2 : map(A, B)]:
         ((![K : A]: lookup(A, B, M1, K) = lookup(A, B, M2, K)) =>
          M1 = M2))).
